...
bool outputIsZip( const std::filesystem::path& outputPath)
{
	std::string pathExtension = outputPath.extension().string();
	std::transform(pathExtension.begin(), pathExtension.end(), pathExtension.begin(), ::toupper);
	if (pathExtension == ".IFCZIP") { return true; }
	return false;
}

void zipFile(const IfcFile& theFile, const std::filesystem::path& outputPath) 
{
	...
	std::string fileContent = theFile.dumptoString();
...
int main(int argc, char* argv[])
{
	...
	if (outputIsZip(outputPath))
	{
		zipFile(theFile, outputPath);
	}
	else
	{
